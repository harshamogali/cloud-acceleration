...
describe('DocDbStack', () => {
  ...
  test('master credentials secret is auto-generated by the cluster (never hardcoded)', () => {
    // We do NOT pass a password into masterUser; CDK creates the secret.
    // The synthesized cluster reference the secret through Fn::Sub, so the
    // password value never appears in the template.
    template.hasResourceProperties('AWS::SecretsManager::Secret', {
      GenerateSecretString: Match.objectLike({
        GenerateStringKey: 'password',
        SecretStringTemplate: Match.stringLikeRegexp('"username":"docdbadmin"'),
      }),
    });
  });

  test('cluster references the secret via dynamic resolution (no hardcoded password)', () => {
    template.hasResourceProperties('AWS::DocDB::DBCluster', {
  ...
  test('automatic password rotation is configured (NIST IA-5)', () => {
