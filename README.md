...
Resources with `RemovalPolicy.RETAIN` are **not** deleted by `cdk destroy` — they must be removed manually to prevent accidental data loss:

- KMS key
- DynamoDB table (`cloud-acceleration-data`)
- DocumentDB cluster (`cloud-acceleration-docdb`) — has `deletionProtection: true`; disable with `aws docdb modify-db-cluster --no-deletion-protection --apply-immediately` before destroying
- Both AWS Backup vaults (compliance lock policy further restricts deletion to the account root principal)
- Secrets Manager secrets (JWT signing key, PagerDuty integration key, DocumentDB master credentials)

CloudWatch log groups use `RemovalPolicy.DESTROY` — they go with their owning stack. Audit retention is enforced via the per-log-group `retention: ONE_YEAR` setting, not via stack lifecycle. This avoids orphan log groups blocking stack re-creation after a rollback (a common cause of repeat deploy failures during initial setup).
...
**5. Configure the API Gateway CloudWatch Logs role** — this is an *account-level* setting required for API Gateway access logging. Without it, the `CloudAccelApi` stack will fail to create with `CloudWatch Logs role ARN must be set in account settings to enable logging`.

```bash
cat > trust-apigw.json <<'EOF'
{
  "Version": "2012-10-17",
  "Statement": [{
    "Effect": "Allow",
...
aws iam create-role --role-name ApiGatewayCloudWatchLogsRole \
  --assume-role-policy-document file://trust-apigw.json
...
This is one-time per account/region, not per stack.
...
- **Retained resources** — KMS key, DynamoDB table, DocumentDB cluster, both backup vaults, and Secrets Manager secrets have `RemovalPolicy.RETAIN`. `cdk destroy` does not remove them; manual cleanup is required. The DocumentDB cluster also has `deletionProtection: true` — disable via the AWS console or CLI before destroying. Log groups use `RemovalPolicy.DESTROY`; they go with their stack and rely on the per-log-group `retention` setting for audit retention.
...
Issues encountered during the initial deploy of this codebase, with concrete remediations. The codebase already incorporates each fix; this section is a reference for diagnosing similar failures in forks or new accounts.

### `Resource of type 'AWS::Logs::LogGroup' with identifier '...' already exists`

A stack rolled back earlier and a named log group survived because of `RemovalPolicy.RETAIN`, blocking the next CREATE.

```bash
aws logs delete-log-group --log-group-name <name>
```

The codebase now uses `RemovalPolicy.DESTROY` on all log groups, so this should not recur. If it does, the orphan is from a deploy predating that change.

### `CloudWatch Logs role ARN must be set in account settings to enable logging`

`CloudAccelApi` is the first stack to enable API Gateway access logging. API Gateway requires an account-level CloudWatch Logs role before any stage in the account can log. See **Continuous Deployment §5** for the one-time setup.

### `Value (...) for parameter GroupDescription is invalid. Character sets beyond ASCII are not supported`

EC2 rejects non-ASCII characters in security group `GroupDescription` and ingress/egress rule descriptions. Em-dashes (`—`) and arrows (`→`) are common offenders. Replace with `-` and `to`. CloudWatch alarm descriptions, IAM role descriptions, and KMS key descriptions accept Unicode — only EC2 fields are strict.

### DocDB `ROLLBACK_FAILED` because cluster won't delete

The cluster has `deletionProtection: true`. CFN cannot delete it during rollback. Disable protection, then retry the stack delete:

```bash
aws docdb modify-db-cluster --db-cluster-identifier cloud-acceleration-docdb \
  --no-deletion-protection --apply-immediately

aws docdb delete-db-cluster --db-cluster-identifier cloud-acceleration-docdb \
  --skip-final-snapshot

# wait for cluster status -> deleted
aws cloudformation delete-stack --stack-name CloudAccelDocDb
```

### DocDB `Property validation failure: PerformanceInsightsRetentionPeriod / PerformanceInsightsKMSKeyId`

`AWS::DocDB::DBInstance` strict-validates against these properties even though the underlying API accepts them. The codebase uses `enablePerformanceInsights: true` only — default 7-day retention with the AWS-managed key. Apply CMK + 31-day retention post-deploy via `aws docdb modify-db-instance` if required.

### PrivateLink: `Response object is too long` from `GetNiDetails`

`AwsCustomResource` returns the full SDK response in the CFN custom-resource payload (4 KB cap). `EC2.DescribeNetworkInterfaces` for 3 ENIs exceeds it. The codebase uses `outputPaths` to filter the response to just the fields read.

### Stack stuck in `ROLLBACK_FAILED`

Subnet groups and parameter groups can't be deleted while the cluster references them. Manually delete the cluster (after disabling deletion protection), then retry `delete-stack`. Alternatively, `continue-update-rollback --resources-to-skip` lets CFN abandon specific resources — but only do this if you're certain the orphans can be cleaned up manually afterwards.

### Curl from laptop hangs against the API endpoint

The API is private (`endpointConfiguration.types: [PRIVATE]`). It is reachable only from inside the provider VPC, or from a consumer account through PrivateLink. Test from EC2/Cloud9 in the VPC, or via the consumer-side VPC Interface Endpoint.

### Workflow run fails immediately at `configure-aws-credentials`

The trust policy on `GitHubActionsCdkDeploy` is locked to `repo:<owner>/<repo>:ref:refs/heads/main`. Pushes to other branches, or runs from forks, will fail to assume the role. Add a separate, more locked-down role for PR builds if you need them.
